...
class CV_EXPORTS_W_SIMPLE Dictionary {
     ...
     */
    bool identify(const Mat &onlyBits, CV_OUT int &idx, CV_OUT int &rotation, double maxCorrectionRate) const;
...
/** @brief Extend base dictionary by new nMarkers
  ...
  */
Dictionary extendDictionary(int nMarkers, int markerSize, const Dictionary &baseDictionary = Dictionary(),
                                         int randomSeed=0);
